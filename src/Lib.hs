...
handleEvent (TimePassing dt) world
  | (not (isNoFlood (worldMap (updateWater world)))) || (isWatered (getXYElem (worldMap world) sinkX sinkY) /= HasWater) = updateWater world
  | otherwise = World {
...
sinkWater prev rows x y from
  ...
  | (isWatered elem) == HasWater = rows
  | (isWatered (getXYElem (checkCurAccessWater prev from x y elem rows) x y)) == Empty = rows
  | (isWatered (getXYElem (checkCurAccessWater prev from x y elem rows) x y)) == Flooded = checkCurAccessWater prev from x y elem rows
  | x == 0 && y == 0 = rows
...
checkCurAccessWater prev LeftDown x y cell rows
  ...
  | (rightDown prev) == Opened = setXYElem rows x y (floodCell cell)
  | otherwise = rows
